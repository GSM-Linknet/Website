...
export const AuthService = {
  ...
  async login(credentials: {
    ...
    try {
      const response = await apiClient.post<ResponseData<LoginResponse>>(
        "/auth/login",
        credentials,
      );
  ...
  async impersonate(targetUserId: string): Promise<LoginResponse> {
    try {
      ...
      const { user, accessToken, refreshToken } = (
        response as { data: LoginResponse }
      ).data;
      ...
      if (!Cookies.get("original_auth_token")) {
        ...
        if (currentToken)
          Cookies.set("original_auth_token", currentToken, { expires: 1 });
        if (currentRefreshToken)
          Cookies.set("original_refresh_token", currentRefreshToken, {
            expires: 7,
          });
        if (currentUserProfile)
          localStorage.setItem("original_user_profile", currentUserProfile);
        if (currentPermissions)
          localStorage.setItem("original_app_permissions", currentPermissions);
      }
  ...
  async stopImpersonation(): Promise<boolean> {
    ...
    const originalPermissions = localStorage.getItem(
      "original_app_permissions",
    );
    ...
    if (originalRefreshToken)
      Cookies.set("refresh_token", originalRefreshToken, { expires: 7 });
    if (originalProfile) localStorage.setItem("user_profile", originalProfile);
    if (originalPermissions)
      localStorage.setItem("app_permissions", originalPermissions);
