...
export default function CustomerListPage() {
    ...
    const handleFilterChange = (key: string, value: string) => {
        ...
        if (newFilters.status !== "all") searchParts.push(`statusCust:${newFilters.status === "active"}`);
        if (newFilters.internet !== "all") searchParts.push(`statusNet:${newFilters.internet === "online"}`);
        if (newFilters.wilayah !== "all") searchParts.push(`idWilayah:${newFilters.wilayah}`);
    ...
    const handleSearch = (val: string) => {
        ...
        if (filters.status !== "all") searchParts.push(`statusCust:${filters.status === "active"}`);
            ...
            <div className="flex flex-wrap items-center gap-3">
                ...
                <FilterDropdown
                    ...
                    options={[
                        ...
                        ...(Array.isArray(wilayahs) ? wilayahs.map(w => ({ label: w.name, value: w.id })) : [])
                    ]}
